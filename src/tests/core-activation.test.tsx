...
describe('Video Studio Creative Orbit activation', () => {
  ...
  it('maps Idle to the entry composite and keeps the Homepage stack out of the ritual', () => {
    const host = document.createElement('div'); const root = createRoot(host)
    const store = useGalaxyStore.getState(); store.completeLoading()
    act(() => root.render(<GalaxyScene/>))
    expect(host.querySelector('.galaxy-scene')?.className).toContain('idle')
    expect(host.querySelector('.galaxy-entry-composite')?.getAttribute('data-scene-role')).toBe('idle')
    expect(host.querySelector('.galaxy-idle-support')?.getAttribute('data-asset-layers')).toBe('stars,atmosphere')
    expect(host.querySelectorAll('.galaxy-idle-support [data-asset-layer]')).toHaveLength(2)
    expect(host.querySelector('.galaxy-home-composite')).toBeNull()
    expect(host.querySelector('.galaxy-home-visuals')).toBeNull()
    expect(host.querySelector('.galaxy-scene')?.getAttribute('style')).toContain('galaxy-master-composite.png')
    act(() => root.unmount())
  })

  it('maps Awakened to the home composite and all eight core-visual layers', () => {
    const host = document.createElement('div'); const root = createRoot(host)
    const store = useGalaxyStore.getState(); store.completeLoading(); store.awaken()
    act(() => root.render(<GalaxyScene/>))
    expect(host.querySelector('.galaxy-home-composite')?.getAttribute('data-scene-role')).toBe('homepage')
    expect(host.querySelector('.galaxy-home-visuals')?.getAttribute('data-asset-layers')).toBe('base,nebula,orbits,coreGlow,coreParticles,exOutline,stars,atmosphere')
    expect(host.querySelectorAll('.galaxy-home-visuals [data-asset-layer]')).toHaveLength(7)
    expect(host.querySelector('.galaxy-asset-base')?.getAttribute('data-scene-role')).toBe('homepage')
    expect(host.querySelector('.galaxy-scene')?.className).toContain('galaxy-scene--home')
    act(() => root.unmount())
  })
  ...
  it('isolates Video Studio from the Hero composite while retaining the independent functional layers', () => {
    ...
    expect(host.querySelector('.galaxy-entry-composite')).toBeNull()
    expect(host.querySelector('.galaxy-home-composite')).toBeNull()
    expect(host.querySelector('.galaxy-home-visuals')).toBeNull()
    expect(host.querySelector('.galaxy-scene')?.getAttribute('style')).not.toContain('galaxy-home-master-composite.png')
    expect(host.querySelector('.galaxy-scene')?.getAttribute('style')).not.toContain('galaxy-master-composite.png')
    act(() => root.unmount())
