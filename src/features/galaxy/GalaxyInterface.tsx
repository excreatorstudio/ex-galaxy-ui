...
const cards = [
  { nameKey: 'cards.luxury', typeKey: 'cards.film', accent: 'estate', cover: projectCovers.film, visual: projectCoverVisuals.film }, { nameKey: 'cards.visual', typeKey: 'cards.ai', accent: 'visual', cover: projectCovers.ai, visual: projectCoverVisuals.ai }, { nameKey: 'cards.brand', typeKey: 'cards.campaign', accent: 'brand', cover: projectCovers.campaign, visual: projectCoverVisuals.campaign }, { nameKey: 'cards.city', typeKey: 'cards.reel', accent: 'city', cover: projectCovers.reel, visual: projectCoverVisuals.reel }, { nameKey: 'cards.soundtrack', typeKey: 'cards.audio', accent: 'audio', cover: projectCovers.audio, visual: projectCoverVisuals.audio },
]
...
export function GalaxyInterface() {
  ...
  return <AnimatePresence>{phase === 'awakened' && <motion.div className="interface home-layout" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: .9, ease: [.16, 1, .3, 1] }}>
    ...
    <section className="floating-cards" aria-label={t('cards.aria')}>{cards.map((card, index) => <motion.button key={card.nameKey} className={`media-card ${card.accent} c${index}`} style={{ '--project-cover-brightness': card.visual.brightness, '--project-cover-hover-brightness': card.visual.hoverBrightness, '--project-cover-saturate': card.visual.saturate, '--project-cover-contrast': card.visual.contrast } as React.CSSProperties} initial={{ opacity: 0, filter: 'blur(5px)' }} animate={{ opacity: 1, filter: 'blur(0px)' }} transition={{ delay: .68 + index * .14, duration: .82, ease: [.16, 1, .3, 1] }}><span className="card-art"><img src={card.cover} alt="" onError={(event) => { event.currentTarget.hidden = true }}/></span><span className="card-meta"><i>{t(card.typeKey)}</i>{t(card.nameKey)}<b>00:{18 + index * 7}</b></span></motion.button>)}</section>
