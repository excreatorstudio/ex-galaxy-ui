...
export function GalaxyInterface() {
  ...
  return <AnimatePresence>{phase === 'awakened' && <motion.div className="interface" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: .9, ease: [.16, 1, .3, 1] }}>
    ...
    <section className="floating-cards" aria-label={t('cards.aria')}>{cards.map((card, index) => <motion.button key={card.nameKey} className={`media-card ${card.accent} c${index}`} initial={{ opacity: 0, y: 36, scale: .94, filter: 'blur(5px)' }} animate={{ opacity: 1, y: 0, scale: 1, filter: 'blur(0px)' }} transition={{ delay: .68 + index * .14, duration: .82, ease: [.16, 1, .3, 1] }}><span className="card-art"><img src={card.cover} alt="" onError={(event) => { event.currentTarget.hidden = true }}/></span><span className="card-meta"><i>{t(card.typeKey)}</i>{t(card.nameKey)}<b>00:{18 + index * 7}</b></span></motion.button>)}</section>
